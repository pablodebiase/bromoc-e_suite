...
! Internal (bonded) energy of chosen particle
!if (Qbond) then
!  if (parl(parn)%ne.gt.1.and.ptypl(parl(parn)%ptyp)%Qpsf) call charmmenergy(parn,einterni)
!  energy=energy+einterni
!endif
